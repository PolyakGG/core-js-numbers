 ...
 */
/** Изначально пробовал в
 *  return Math.sqrt(x1 ** 2 - y1 ** 2 + (x2 ** 2 - y2 ** 2));
 */
 ...
 *   (0,1) (0,1)     => 0
 */
/** Заюзал здесь let заместо const т.к. она не изменяется потом(константа))) */
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const odin = Math.atan2(y1, x1);
  const dva = Math.atan2(y2, x2);
  let tri = dva - odin;
  if (tri < 0) {
    tri += 2 * Math.PI;
  }
 ...
 * '-525.5'     => -525.5
 */
/** Вернуть число из строки (parseInt не сработал надо radix parameter ) */
 ...
 */
/**
 * Число которое округлить num
 * степерь 10-ти до которой округлить
 * 10 в степени pow
 * деление сдвигает точку влево на pow знаков
 * Math.round округляет
 * 10 в степени pow сдвигает точку вправо
 */
 ...
 */
/** Делаю переменную va которая преобразует стринг в инт value
 * потом если не преобразовалось то Nan(Not a number) и если да то оно возвращает def, если нет, то va = value
 */
function toNumber(value, def) {
  const va = parseInt(value, 10);
  return Number.isNaN(va) ? def : va;
}
...
function numberToStringInBase(number, base) {
  const rad = number.toString(base);
  return rad;
}
...
function toExponential(number, fractionDigits) {
  const exponenta = number.toExponential(fractionDigits);
  return exponenta;
}
